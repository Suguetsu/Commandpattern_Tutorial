...
public class UserControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isOut)
        {
            Debug.Log(m_TargetUnit.Side + " , " + m_TargetUnit.CurrentCell);
            var a = m_TargetUnit;

            CaptureCommand cmd2 = new CaptureCommand(a, a.CurrentCell);
            CommandManager.Instance.AdOut(cmd2);
    ...
    void MoveUnit()
    {
         ...
         out Vector3Int clickedCell))
        {
            ...
            if (m_MovableCells.Contains(clickedCell))
            {
                ...
                else if (unit.Side != Gameboard.Instance.CurrentTeam)  // Identify if the cell isen't from the same team
                {
